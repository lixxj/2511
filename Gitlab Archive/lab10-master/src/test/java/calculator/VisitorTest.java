...
public class VisitorTest {
    ...
    public void testModulus2() {
        Parser parser = new Parser("4 % 3"); 
	    TreeNode node = parser.parse();
        EvaluatorVisitor evaluatorVisitor = new EvaluatorVisitor();
        node.accept(evaluatorVisitor);
        assertEquals(1, evaluatorVisitor.getValue());
    }

    @Test
    public void testModulus3() {
        Parser parser = new Parser("3 * 4 % 3"); 
	    TreeNode node = parser.parse();
        EvaluatorVisitor evaluatorVisitor = new EvaluatorVisitor();
        node.accept(evaluatorVisitor);
        assertEquals(0, evaluatorVisitor.getValue());
    ...
    public void testModulus4() {
        ...
        EvaluatorVisitor evaluatorVisitor = new EvaluatorVisitor();
        node.accept(evaluatorVisitor);
        assertEquals(4, evaluatorVisitor.getValue());
    }
}