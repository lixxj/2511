...
public class SimpleTableView<E> implements TableView<E> {
    ...
    public int count() {
        ...
        for ( ; hasNext() ; ++i ) next();
        return i;
        // return (int) StreamSupport.stream(parent.spliterator(), false).count();
    }
    ...
    public <R> R reduce(BiFunction<R, E, R> reducer, R initial) {
        ...
        while (true) {
            ...
            // Each worker thread can process its own reduction, it only needs the next value to be synchronised
            curr = reducer.apply(curr, next);
        }
        return curr;
    }
