...
public class SimpleTableView<E> implements TableView<E> {
    ...
    public <R> R reduce(BiFunction<R, E, R> reducer, R initial) {
        ...
        // The problem caused synchronized is for (var val : this) since the iterator is single-direction.
        // Extra hint for anyone confused for this, is using a while loop first to continuous checking this.hasNext()

        while (true) {
            E next;
            synchronized(this) {
                if (this.hasNext()) next = next();
                else break;
            }
